...
function GeneralAdviceEdit_KH() {
  ...
  const fetchDetail = async () => {
    const res = await callAPI({ method: "post", endpoint: "/generaladvices_kh/detail", data: { id } });
  ...
  const handleEdit = async () => {
    try {
      await callAPI({
        method: "post",
        endpoint: "/generaladvices_kh/edit",
        data: {
          id,
          maHoSo,
          idKhachHang,
    ...
    <div className="p-1 bg-gray-100 flex items-center justify-center">
      <div className="bg-white p-4 rounded-lg shadow-md w-full max-w-4xl">
        <h2 className="text-2xl font-semibold text-gray-700 mb-4">
          ✏️ Cập nhật tư vấn chung Campuchia
        </h2>
...
export default GeneralAdviceEdit_KH;
