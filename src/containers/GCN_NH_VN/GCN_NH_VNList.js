...
const PAGE_STORAGE_KEY_GCN_VN = "gcnNhVnListPage";
...
function GCN_NH_VNList() {
  ...
  const fetchGCNs = async (
    ...
    try {
      localStorage.setItem(PAGE_STORAGE_KEY_GCN_VN, page.toString());
      ...
      const response = await callAPI({
        ...
        data: {
          ...
          partnerName: pn,
          brandName: bn,
          // nếu API có filter maQuocGia thì thêm countryCode ở đây
          pageSize: size,
          pageIndex: page,
        },
      });
      ...
      console.error("Lỗi khi lấy dữ liệu GCN VN:", error);
    }
  };
  ...
  useEffect(() => {
    const savedPage = parseInt(
      localStorage.getItem(PAGE_STORAGE_KEY_GCN_VN) || "1",
      10
    );
    const savedFiltersString = localStorage.getItem(FILTER_STORAGE_KEY_GCN_VN);
    ...
    if (navigationType === "POP" && savedStateString) {
      ...
      // 👉 Vào mới / PUSH: lấy filter (nếu có) và gọi API
      if (savedFiltersString) {
        try {
          ...
          console.error("Error parsing saved filters (GCN_VN)", e);
          fetchGCNs("", "", savedPage, pageSize);
        }
      } else {
        fetchGCNs("", "", savedPage, pageSize);
    ...
    if (!localStorage.getItem(PAGE_STORAGE_KEY_GCN_VN)) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      STATE_STORAGE_KEY_GCN_VN,
      JSON.stringify(stateToSave)
    );
  }, [gcn_nhs, totalItems, pageIndex, pageSize]);

  // Xử lý xóa (nếu sau này dùng)
    ...
    <div className="p-1 bg-gray-100 min-h-screen">
      <div className="bg-white p-4 rounded-lg shadow-md">
        <h2 className="text-2xl font-semibold text-gray-700 mb-4">
          📌 Danh sách giấy chứng nhận (văn bằng)
        </h2>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {/* Nếu muốn cho lọc quốc gia luôn thì mở comment này */}
          {/* <div className="w-full md:w-1/4">
            <label className="block text-sm font-medium text-gray-700 mb-1 text-left">
              Quốc gia
            </label>
          ...
          <tbody>
            {gcn_nhs.map((gcn_nh, index) => (
              ...
              >
                ...
                <td className="p-2 text-table">{gcn_nh.maQuocGia}</td>
                  ...
                  {(role === "admin" || role === "staff") && (
                    <div className="hidden group-hover:flex gap-2 absolute right-2 top-1/2 -translate-y-1/2 bg-white p-1 rounded shadow-md z-10">
                      <button
                        ...
                        onClick={() => navigate(`/gcn_nh_vnedit/${gcn_nh.id}`)}
                      >
                        📝
                      </button>
                    </div>
